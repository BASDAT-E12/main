...
def get_rapat():
    cur.execute("SELECT t.id_pertandingan, STRING_AGG(nama_tim, ' VS ') as tim, o.nama_depan || ' ' || o.nama_belakang as nama_panitia, s.nama as nama_stadium, r.datetime, r.isi_rapat FROM RAPAT R JOIN PERTANDINGAN P ON R.ID_PERTANDINGAN = P.ID_PERTANDINGAN JOIN STADIUM S ON P.STADIUM = S.ID_STADIUM JOIN TIM_PERTANDINGAN T ON P.ID_PERTANDINGAN = T.ID_PERTANDINGAN JOIN PANITIA N ON N.ID_PANITIA = R.PERWAKILAN_PANITIA JOIN NON_PEMAIN O ON O.ID = N.ID_PANITIA GROUP BY T.id_pertandingan, nama_panitia, s.nama, r.datetime, r.isi_rapat")
    result = namedtuplefetchall(cur)
    return result 
